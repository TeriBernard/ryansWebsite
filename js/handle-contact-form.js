...
function handleContactForm(formVals, formElement) {
    ...
    emailAjaxData(dataWithFormats).then(function (resData) {
        // Hide spinner
        statusLoading.classList.remove(fadeReveal);
        if (resData === null || (typeof resData === 'undefined' ? 'undefined' : _typeof(resData)) !== 'object' || !resData['status']) {
            console.log(resData);
            ...
            if (resData['status'] === 'success') {
                ...
                // Show emailed values
                var nameResult = document.getElementById('sent-name').innerHTML = resData.data.sanitized.name;
                var phoneResult = document.getElementById('sent-phone').innerHTML = resData.data.sanitized.phone;
                var emailResult = document.getElementById('sent-email').innerHTML = resData.data.sanitized.email;
...
function setInvalid(failKey, inputElem) {
    ...
    if (failKey) inputElem.classList.add(invalidClass);else inputElem.classList.remove(invalidClass);
}